...
COPY file/etc/yum.repos.d/CentOS* /etc/yum.repos.d/
RUN dnf install -y epel-release centos-release-ceph-pacific

COPY file/etc/yum.repos.d/epel* /etc/yum.repos.d/
COPY file/etc/yum.repos.d/CentOS-Ceph-Pacific.repo /etc/yum.repos.d/
...
RUN dnf install -y libnsl2 libffi bzip2 readline ncurses gdbm tcl libuuid zlib zlib xz tk openssl sqlite glibc
